...
Policy tensorProduct(Policy policy1, Policy policy2) {
  ...
  List<Node> createNodesFromCombined(List<_CombinedNode> combinedNodes) {
    return combinedNodes.map((node) {
      ...
      return node.type == NodeType.tag ? TagNode(label, position) : Node.fromType(node.type, label, position);
    }).toList();
  }
  ...
  final combinedNodes = cartesianProduct(policy1.nodes, policy2.nodes)
  ...
  final nodes = createNodesFromCombined(combinedNodes);
  final edges = createEdgesFromCombined(combinedEdges, combinedNodes, nodes);
